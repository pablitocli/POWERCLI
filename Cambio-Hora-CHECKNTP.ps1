...
##VARIABLES!!
$OutputFileName	= "D:\inventario\$fecha\host\Cambio-hora-NTP-HOST.html"
$ReportTitle	= "Verificación de Servicio de NTP TIME de la Infraestructura Virtual -  Administración Cloud YPF "
$Propsh = @()
$Resultshost = @()
$x= @()
##CSV DE SALIDA
$outputstore = "D:\inventario\$fecha\host\Cambio-hora-NTP-HOST.csv"
...
if ($x -ne $null)
		...
		IF	($timedife -gt "60")
		{
		Write-Host "<<<<<<<<<<<<<Reiniciando servicio de NTP en: " $esx.vmhost" >>>>>>>>>>>>>>>>>>>>>>>>"
		$ntpservices | stop-VMHostService -confirm:$false
		sleep 10
